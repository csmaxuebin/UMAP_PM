...
# mean = np.average(label_test)
# print("mean",mean)
result = Counter(label_test)
result_list=list(result.values())
print(result_list)
gailv=result_list[1]/sum(result_list)
print (gailv)
eps=10
...
print(LR2_sum_list)
LR2_gailv=LR2_sum_list[1]/sum(LR2_sum_list)
...
with open('file-Adult_PM_gailv.csv', 'a') as fid:
...
# cnt_one = np.sum(LR2_pred_labels)
# est_one = random_response_adjust(sum=cnt_one, N=len(LR2_pred_labels), epsilon=eps)
# est_mean = est_one / len(LR2_pred_labels)
